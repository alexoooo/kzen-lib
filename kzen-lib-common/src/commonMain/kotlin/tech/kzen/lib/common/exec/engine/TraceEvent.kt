 ...
 * the live latest-value view ([Node.live]) is a projection of it — last write per [address], minus any
 * values a re-running scope has since reset ([Execution.resetEmitted]). Both write modes therefore come
 * from a single stream — there is no second parallel structure.
 *
