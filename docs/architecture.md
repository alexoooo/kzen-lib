...
| **Data model** | Structural and JVM-native contracts (value constraints — text symbol sets, JVM enums included — ride path-aligned beside native metadata in `constraintsByPath`, enforced by `validate` and invisible to the type algebra), backing-independent values, bounded snapshots, and named Logic bindings. `BindingState.Unbound` is distinct from a bound null value; defaults are applied once when executable inputs are bound. | `DataType`, `DataContract`, `DataConstraint`, `DataValue`, `DataSnapshot`, `BindingSchema`, `DataBindings` |
