...
| **Data model** | Structural and JVM-native contracts (value constraints — text symbol sets, JVM enums included — ride path-aligned beside native metadata in `constraintsByPath`, enforced by `validate` and invisible to the type algebra), backing-independent values, bounded snapshots, and named Logic bindings. A value is a payload plus optional **metadata** (`DataValue.metadata`, a `ValueMetadata`, typed by `DataContract.metadata`, a `MetadataContract`): a non-nullable record of plain data only — no opaque or native members, no metadata of its own (`ValueMetadata`, `MetadataContract` and `MetadataSnapshot` — the value, type and snapshot sides — have no slot for nested metadata, so it never nests; their `of` factories fail by name on a record that carries it) — so it never keeps a payload's resources alive, snapshots and serializes with the value, and is shared by the values derived from one input; `payload()` / `withMetadata` split and pair the two, and `DataOverlay` composes a value lazily without copying. `BindingState.Unbound` is distinct from a bound null value; defaults are applied once when executable inputs are bound. | `DataType`, `DataContract`, `MetadataContract`, `DataConstraint`, `DataValue`, `ValueMetadata`, `DataOverlay`, `DataSnapshot`, `MetadataSnapshot`, `BindingSchema`, `DataBindings` |
