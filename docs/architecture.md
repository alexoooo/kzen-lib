...
| **Execution** | The entire surface a Logic touches, handed to it by the engine. Boundaries (`checkpoint`), tracing (`emit` / `log` / `resetEmitted`), composition (`host`), resources (`declareExport` / `resource` / `resourceValue` / `hasResourceInFamily` / `releaseResource`), interaction (`onRequest`), and live-edit state (`onCapture` / `restored` / `moveTarget`). | `Execution`, `Address`, `PauseReason`, `ClosePolicy` |
